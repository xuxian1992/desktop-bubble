...
**[→ Releases 页面](https://github.com/xuxian1992/desktop-bubble/releases)**

| 形态 | 说明 |
|---|---|
| **免安装版**（zip） | **推荐**。解压后双击 `START.vbs`，删目录即卸载 |

## 关于未签名

**这是一个个人开源项目，不做代码签名。** 所以下载的东西会带上「未知发布者」的标记。

下载时可能遇到两种拦截，**处理方式完全不同**：

| 拦截 | 表现 | 怎么办 |
|---|---|---|
| **SmartScreen** | 「Windows 已保护你的电脑」 | 点「更多信息」→「仍要运行」 |
| **智能应用控制（SAC）** | 「已阻止可能不安全的应用」 | **没有「仍要运行」这个选项**，见下 |

### 遇到智能应用控制怎么办

SAC 拦的是**未签名的新 exe**。所以：

1. **用免安装版**。它的 `electron.exe` 是 **Electron 官方原版**（SHA256 与官方一致），
   有广泛的使用信誉，通常能通过；而安装包形态的 exe 是被改过的、全新的，基本必被拦。
2. 或者关掉它：`设置 → 隐私和安全性 → Windows 安全中心 → 应用和浏览器控制 → 智能应用控制`
   ⚠️ 这个开关历史上是**单向**的（重开需重装系统）
3. 或者[从源码运行](#从源码运行) —— 用你本机的 Node 跑，不涉及任何打包出来的 exe
## 这是什么
...
- **未签名**：Windows 会提示未知发布者。若开启了**智能应用控制**，未签名应用会被**直接拦截且无法绕过**（需要代码签名证书，或从源码运行）
